export interface registrarcuentacontabilidad {
    // columna db 
    id?: number,
    cuenta: string,
    tipo:string,
    }

    export interface cuentacontabilidadInter {
        // columna db 
        id?: number,
        cuenta: string,
        tipo:string,
        }

    
        export interface ActivoInter {
            // columna db 
            id?: number,
            nivel: number,
            nombrecuenta:string,
            cuentamayor:number,
            activonivel:string,
            activoniveln2:string,
            totalnum:string,
            numero:string,
            
            }

            export interface ActivoIntern3 {
                // columna db 
                id?: number,
                nivel: number,
                nombrecuenta:string,
                cuentamayor:number,
                totalnum:string,
                numero:string,
                
                
                }


                
            export interface ActivoIntern4 {
                // columna db 
                id?: number,
                nivel: number,
                nombrecuenta:string,
                cuentamayor:number,
                totalnum:string,
                numero:string,
                }

                export interface ActivoIntern5 {
                    // columna db 
                    id?: number,
                    nivel: number,
                    nombrecuenta:string,
                    cuentamayor:number,
                    totalnum:string,
                    numero:string,
                    
                    }

                    export interface ActivoIntern6 {
                        // columna db 
                        id?: number,
                        nivel: number,
                        nombrecuenta:string,
                        cuentamayor:number,
                        totalnum:string,
                        numero:string,
                        
                        }

    //PASIVO INTER

    export interface PasivoIntern2 {
        // columna db 
        id?: number,
        nivel: number,
        nombrecuenta:string,
        cuentamayor:number,
        activonivel:string,
        activoniveln2:string,
        totalnum:string,
        numero:string,
        
        }


        export interface PasivoIntern3 {
            // columna db 
            id?: number,
            nivel: number,
            nombrecuenta:string,
            cuentamayor:number,
            totalnum:string,
            numero:string,
                       
            }

            export interface PasivoIntern4 {
                // columna db 
                id?: number,
                nivel: number,
                nombrecuenta:string,
                cuentamayor:number,
                totalnum:string,
                numero:string,
                
                
                }

                export interface PasivoIntern5 {
                    // columna db 
                    id?: number,
                    nivel: number,
                    nombrecuenta:string,
                    cuentamayor:number,
                    totalnum:string,
                    numero:string,
                    
                    
                    }


                    export interface PasivoIntern6 {
                        // columna db 
                        id?: number,
                        nivel: number,
                        nombrecuenta:string,
                        cuentamayor:number,
                        totalnum:string,
                        numero:string,
                        
                        
                        }