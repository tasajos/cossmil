...
import { egresoInter,opcionesegresoInter } from 'src/app/Interfaz/transacciones';
import { ActivatedRoute, Router } from '@angular/router';


@Component({
  selector: 'app-egresost',
  templateUrl: './egresost.component.html',
  styleUrls: ['./egresost.component.css']
})
export class EgresostComponent implements OnInit {
  formulario: FormGroup;
  numero!: number;
  opcionesCuentas: opcionesegresoInter[] = [];
  ...
  registrarcuenta() {
    ...
    this._ringresosservice.postegresocuentas(rcactivo).subscribe(_data => {
      this.mensajeExito('registrado');
      location.reload();
      this.router.navigate(['/egresost']);;
  ...
  registrarnrorecibo() {
    ...
    const rnros: opcionesegresoInter = {
     numero: parseInt(this.formulario.value.numero),
     
   };

 // Enviamos objeto al backend
 this._rnroreciboservice.postopcioneegresocuentas(rnros).subscribe(_data => {
  this.numero = this.numero + 1;
  ...
  ngOnInit(): void {
    ...
    this._ringresosservice.getopcionescuentasegreso().subscribe((opcionesCuentas: opcionesegresoInter[]) => {
...
obtenerUltimoNumero() {
  ...
  this._ringresosservice.getUltimoNumeroe().subscribe((ultimoNumero: number) => {
